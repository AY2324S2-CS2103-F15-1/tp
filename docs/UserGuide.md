...
4. *Today's Meeting List*: The list of persons who have a meeting on the current date of your computer when running the application (if any). Note that this list will be display the latest list when a command to update the list is **executed successfully**.
